...
import classes from './RssView.css'
...
class RssView extends Component {
	...
	render() {
		const {
			feeds, userFeeds, activeAmount, activeId, activeTitle, activeLink,
			reloadFeed, removeFeed, toggleExpands, expands, editFeed} = this.props;
		...
		if (arrayFeeds) {
			currentFeed = arrayFeeds.map((item, index) => {
				...
				return <div key={index}>
					{item.enclosure && <img className={classes.Images} src={item.enclosure.url} alt={item.title}/>}
					<h5 className={classes.Title}>
						<a href={item.link} target="_blank"><i
							className={classes.LinkItemIcon + ' fas fa-link'}/> {item.title}</a></h5>
					{item.categories && <p className={classes.Categories}>{item.categories.join(', ')}</p>}
					{item.pubDate && <p className={classes.Date}>{textDate}</p>}
					{item.author && <p className={classes.Author}>{item.author}</p>}
					{item.content && <p className={classes.Content + " cont"}>{Parser(item.content)}</p>}
		...
		return (
			<div className={expands ? classes.RssReaderFull : 'col-md-9'}>
				...
				{activeTitle &&
				<header>
					<h5 className={classes.HeaderTitle}>{activeTitle} <span
						className={classes.Amount}>({activeAmount})</span></h5>
				...
				{feeds.length !== 0 ?
					<div className={classes.RssReaderListWrap + ' shadow-lg'}>
						...
						<Scrollbars
							...
							<div className={classes.RssReaderView}>
								{currentFeed}
							</div>
						</Scrollbars>
					...
					:
					userFeeds.length !== 0 && activeTitle === '' &&
					<p className={classes.ListEmpty}>{'<'} Feed not selected. Select one of the RSS feeds and
						click on its name</p>
				}

				{userFeeds.length === 0 &&
				<p className={classes.ListEmpty}>You don't have RSS feeds! Add the first link
					by clicking on the button - Add Feed</p>}
