...
import classes from './RssEdit.css';
...
class RssEdit extends Component {
	...
	render() {
		const {editFeed, title, link, updateField, toggleModal, isInvalid} = this.props;

		return (
			<div className={classes.RssEdit}>
				<h4 className="text-center">Edit Feed - {title.substr(0, 15) + (title.length > 15 ? '...' : '')}</h4>
				...
				<form onSubmit={editFeed}>
					...
					<div className="text-center">
						<button className={classes.SaveBtn + ' btn btn-success'} disabled={isInvalid} type="submit">Save
							Feed
						</button>
