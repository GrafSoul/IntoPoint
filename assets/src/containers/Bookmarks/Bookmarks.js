...
class BookmarksPage extends Component {
	...
	handleAddBookmark = (event) => {
		const {authUser, link, title, tags, bookmarksInitial, updateField} = this.props;
	...
	handleEditBookmark = (event) => {
		const {authUser, idBookmark, link, title, tags, bookmarksInitial, updateField} = this.props;
	...
	render() {
		const {
			link, title, tags, chips, error, updateField, idBookmark, searchTags,
			searchTagsList, searchValue, userBookmarks, userTags, filter, modal, isEdit, toggleModal
		} = this.props;
				...
				<div className="container">
					<div className="row">
						...
						<div className="col-6 text-lg-right col-md-3">
							<button className="btn btn-success" onClick={this.handleModalAddBookmark}>
...
const mapDispatchToProps = (dispatch) => ({
	...
	searchTagsList: (value, tags) => dispatch(actionCreators.searchTagsList(value, tags)),
	toggleModal: () => dispatch(actionCreators.toggleModal()),
	bookmarkEdit: (id, link, title, tags) => dispatch(actionCreators.bookmarkEdit(id, link, title, tags))
});
