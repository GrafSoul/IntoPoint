...
const NotesList = (props) => {
	...
	const notesList = notes ? notes.map((item) => {
		let noteTags = (item.tags).replace(/\s+/g, '').split(',');
		let notesItem = <NotesItem
			key={item.key}
			idNote={item.key}
			text={item.text}
			title={item.title}
	...
	return (
		<div className="col-md-9">
			...
			{notes.length !== 0 ?
				...
				:
				<p className={classes.ListEmpty}>You have no saved note! Add the first note by clicking on the button
					- Add Note</p>
			}
